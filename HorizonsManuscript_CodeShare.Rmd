...
anova(glmer_explore,glmer_explore.horizons)
```
...
m_expt.ageXh<-glmer(exploit~agegroup*
                      as.factor(horizons_all)+(1|id),
                    data=subset(dtc1,other==0),
...
m_expt.age_h<-glmer(exploit~agegroupa + h_short + agegroup11 + h_long+(1|id),
                    data=subset(dtc1,other==0),
                    family="binomial")
...
# #save
# jpeg("Horizons_fig3.jpg")
# fig3
# dev.off()
```
Stats:
```{r}
glmer_explore.size<-glmer(explore~reward_exploit+(1|id),
                          data=subset(dtc1,other==0),
                          family="binomial")
...
figX2<-ggplot(data=dtc1,aes(x=reward_exploit,
  ...
  geom_smooth(method="lm") +
  labs(x="reward size of exploit option in trial",
       y="proportion of explore initial choices") +
...
```{r}
glmer_explore.sizeXage<-glmer(explore~scale(reward_exploit)*agegroup+
                                (1|id),
                          data=subset(dtc1,other==0),
                          family="binomial")
...
glmer_explore.size_age<-glmer(explore~reward_exploit+agegroup +(1|id),
                          data=subset(dtc1,other==0),
                          family="binomial")
...
m2<-glmer(explore~agegroup*h_long + 
            (h_long|id),
          data=subset(dtc1, h_amb == 0 &
                        other==0),
          control=glmerControl(
...
m_RewardAdaptXAge<-lm(reward_total~adaptation*agegroup11+adaptation*agegroupa,data=dtsum)
...
fig7<-dtc1 %>% filter(horizons_all==0) %>%
  ...
  ggplot(aes(x=factor(agegroup,
             ...
             colour=factor(agegroup,
                     ...
                     labels=c("Younger Children",
                              "Older Children",
                              "Adults")))) +
    geom_violin(size=1) + geom_boxplot(width=0.1,size=.8)+
  geom_jitter(shape=15,position=position_jitter(0.2),size=3)+ 
  #geom_hline(yintercept=.5,linetype=2,color="grey20",size=1.5)+#add chance line
  scale_fill_hue(c=20)+ #reduce saturation and increase luminance
...
Stats: Do exploit choices differ across age groups in ambiguous horizons?
```{r}
...
### Reward size
Is age group difference in exploit choices driven by the exploit reward size effect?
...
Controlling for reward size, does age still affect exploit choices in ambiguous horizons?
```{r}
#model w/o interaction
glmer_amb.age_reward<-glmer(exploit~
                       reward_exploit + agegroup +
                       (1|id), 
                 data=subset(dtc1,h_amb==1 & other==0), 
                 family=binomial)

summary(glmer_amb.age_reward)
```
...
#compare 
anova(glmer_amb.ageXreward,glmer_amb.age_reward)
```
